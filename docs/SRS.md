...
| FR-2.4a | Admin can reset another operator account's password (no self-service password recovery exists, since there is no self-registration or email flow) | Medium | Not started |
| FR-2.5 | Operator can update their account information (username) | Medium | Not started |
