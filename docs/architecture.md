...
- Module seeds provide deterministic, production-plausible baseline data through the centralized seed lifecycle. One natural corpus shared by dev, integration, and E2E environments, never test-only fixtures.
...
- All E2E records come from the API-owned deterministic natural seed against an isolated database before each isolation scope. Playwright owns no record definitions.
