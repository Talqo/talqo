...
function findEmbedScript(): HTMLScriptElement | null {
	const scripts = document.querySelectorAll<HTMLScriptElement>("script[data-talqo-widget], script[data-talqo-agent]")
	if (scripts.length > 1) {
...
function render(appearance: WidgetAppearanceInput, agentId: string | undefined, hidden: boolean) {
	...
	root?.render(
		<EmbeddedWidget
			title={dataset?.talqoTitle}
			agentId={agentId ?? dataset?.talqoAgent}
...
export function mount(target: MountTarget = DEFAULT_TARGET) {
	...
	// Legacy `data-talqo-agent` snippets and the dev harness have nothing to fetch, so
	// they paint immediately -- which is also what keeps bundle.test.ts network-free.
